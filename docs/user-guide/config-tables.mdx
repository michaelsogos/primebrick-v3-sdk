...
| `single_select` | string, validated against API or values_source | `ComboSelect` (single) | `type_config` has `api_url` or `values_source`, `value_field`, `label_field`. Was `list`. |
| `multi_select` | comma-separated string | `ComboSelect` (multi) | Same `type_config` as `single_select`. Value stored as CSV (e.g. `"EUR,USD,GBP"`). |
...
| `single_select` / `multi_select` | `{"api_url":"...","value_field":"...","label_field":"..."}` or `{"values_source":"currencies","value_field":"...","label_field":"..."}` | `{"api_url":"/api/v1/...","value_field":"uuid","label_field":"name"}` |
...
- **Right column**: a widget chosen by `entry.type`:
  ...
  - `single_select` → `ComboSelect` single mode (options from `type_config.api_url` or `type_config.values_source`)
  - `multi_select` → `ComboSelect` multi mode (same options source; value stored as CSV)
