...
enforceHttpRbac(user, [Permission.CUSTOMER_READ_ALL, Permission.CUSTOMER_READ_SINGLE]);
// OR semantics by default — user needs ANY of the listed permissions

enforceHttpRbac(user, [Permission.CUSTOMER_CREATE_SINGLE], "all");
// AND semantics — user needs ALL listed permissions
```

Admin users (`isAdmin=true`) and system API keys (`isSystem=true`) bypass all
permission checks. Wildcard patterns in role mappings (e.g. `customer.read.*`)
